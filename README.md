...
ログインには、初回セットアップで自動的に作られる管理者アカウントを使ってください。

| ユーザー名 | パスワード | ロール |
|---|---|---|
| `admin` | `password` | ADMIN |

一般ユーザーの画面を見たい場合は `/register` から別のアカウントを作成できます
（登録されるのは `role: 4` なので、ユーザー管理画面には入れません）。

> 管理者アカウントが消えてしまった場合は、コンテナの中で `bun run seed:admin` を実行すれば作り直せます。
