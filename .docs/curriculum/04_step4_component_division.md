...
ブランチ名・ビルド・HeroUI v3 の書き方など、**機械的に確認できることだけ**を検査します。
ここで見つかるのは「間違いようがない」種類の問題なので、**自分で直してから**提出してください
（このチェックは代わりに直しません。直し方が分からなければ聞いてください）。

> 通っても「合格」ではありません。実装の中身はレビューで一緒に見ます。
> 特に**自分が書いたコードを説明できるか**は、機械では確認できません。必ず自分で確かめてください。

チェックが通ったら、GitHub で **PR** を作成してレビューを依頼します。

| | |
|---|---|
| **from** | `{自分の名前}-step-N`（作業ブランチ） |
| **to** | `{自分の名前}`（自分のベースブランチ） |

> ⚠️ **`main` に向けないでください。**

PR を作るとビルドとテストが自動で走ります。**緑になったことを確認してから**依頼してください。

これらが完了したら、**本カリキュラムは修了**です。おつかれさまでした。
[参考実装](../../README.md)（作成者が書いた実装例のひとつ）と見比べてみると、
同じ要件でも作り方に幅があることが分かると思います。違っていて構いません。
気になった差分について「なぜそうしたか」を考えてみると、より身につきます。
